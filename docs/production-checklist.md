...
- [ ] Configure Groq server env variables: `GROQ_API_KEY`, `GROQ_MODEL`, `GROQ_BASE_URL`.
- [ ] Configure email server env variables: `RESEND_API_KEY`, `EMAIL_FROM`, `ADMIN_NOTIFICATION_EMAIL`.
- [ ] Confirm no `VITE_GROQ_API_KEY`, `VITE_RESEND_API_KEY`, or `VITE_SUPABASE_SERVICE_ROLE_KEY` exists.
...
- [ ] Browser bundle contains no `GROQ_API_KEY`.
- [ ] Groq prompt-injection test refuses secret/system-prompt disclosure.
- [ ] Queries like black vest, budget vest, summer pajama, and Hinglish requests return real product IDs only.
- [ ] Groq failure falls back to local product discovery without crashing the app.
